...
from .batch import (
    ...
    _load_pil,
    _scan_images,
    directory_signature as _directory_signature,
)
...
def _start_is_changed(**kwargs) -> Any:
    """目录指纹 + 循环序号：换源文件夹会自动重跑，不用重启、不用改批次号。"""
    ...
    sig = _directory_signature(directory, include_subdir, filter_keyword, sort)
    return (sig, _loop_index_changed(**kwargs))
